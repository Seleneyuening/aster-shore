    ...
    <section className="px-5 pb-20 pt-0">
      <div className={all ? "mx-auto max-w-7xl" : "home-panel mx-auto max-w-7xl border-t-0 p-8"}>
        <div className="mb-7 flex items-end justify-between gap-6">
          ...
          <Link className="inline-flex items-center gap-2 text-sm text-shore-mist" href="/journal">View all <ArrowRight className="h-4 w-4" /></Link>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {visible.map((post) => (
            <Link href="/journal" className="group overflow-hidden border border-white/12 bg-[#0b1736]/72 shadow-[0_20px_80px_rgba(0,0,0,.24)] backdrop-blur-xl transition hover:-translate-y-1" key={post.title}>
